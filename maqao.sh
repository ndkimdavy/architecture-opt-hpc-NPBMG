...
COMPARE_BINARIES=(
    # Compiler Duels
    # 1. MPI(f90_aoc) vs MPI(f90_gfortran)
    # 2. OMP(f90_aoc) vs OMP(f90_gfortran)
    # 3. OMP(cpp_aoc) vs OMP(cpp_clang++)
    # 4. OMP(cpp_aoc) vs OMP(cpp_g++)
    ...
    # 1. OMP(f90_aoc) vs OMP(cpp_aoc)
    # 2. MPI(f90_aoc) vs OMP(f90_aoc)
