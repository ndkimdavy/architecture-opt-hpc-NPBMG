...
\begin{itemize}
    \item La fonction \texttt{resid :} Elle représente 29,57 \% de la couverture totale. Le calcul est dominé par la Loop 37 (18,68 \%) et la Loop 38 (12,37 \%).
    \item La fonction \texttt{psinv :} Cette routine représente 11,68 \% du temps. Elle est portée par la Loop 77 (7,31 \%) et la Loop 78 (4,72 \%).
...
\vspace{0.5cm}
Le duel entre les deux langages optimisés confirme la supériorité structurelle du Fortran pour ce type de benchmark scientifique. En effet, alors que la version Fortran (\texttt{r0}) s'établit à un temps performant de 11,83 s, la version C++ (\texttt{r1}) accuse un retard notable avec 19,48 s. 
...
Cet écart s’explique en partie par une \textit{Array Access Efficiency} plus élevée en Fortran (97,2~\%) ainsi que par un score de compilation optimal (100~\%). Ces indicateurs suggèrent que le compilateur exploite plus efficacement le modèle mémoire et les structures de données propres au Fortran. À l’inverse, la version C++ semble conserver un overhead structurel, malgré les optimisations manuelles appliquées.
