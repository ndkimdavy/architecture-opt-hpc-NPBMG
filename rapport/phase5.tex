...
\paragraph{Note sur l'expérience Fortran :} Bien que le Fortran soit un langage ancien, il demeure une référence majeure et historique dans le calcul scientifique pour sa gestion native des tableaux multidimensionnels. Plus habitués aux optimisations en C/C++, nous avons également appliqué les recommandations de MAQAO à la version d'origine en Fortran pour observer si l'analyse CQA pouvait encore améliorer un langage déjà hautement optimisé par nature, afin de comparer l'évolution des rapports de performance.
...
\begin{itemize}
    \item La fonction \texttt{resid} : Elle représente 29,57 \% du temps total d’exécution. Le calcul est dominé par la Loop 37 (18,68 \%) et la Loop 38 (12,37 \%).
    \item La fonction \texttt{psinv} : Cette routine représente 11,68 \% du temps total d’exécution. Elle est portée par la Loop 77 (7,31 \%) et la Loop 78 (4,72 \%).
    \item La fonction \texttt{interp} : Bien que moins coûteuse (5,42 \%), elle présentait des problèmes d'accès mémoire à pas non unitaires (\textit{strided access}) signalés par MAQAO.
\end{itemize}

\vspace{0.5cm}
Malgré nos modifications manuelles, le score de compilation (\textit{Compilation Options Score}) du C++ stagne à 16,7 \%. Ce score ne traduit pas une absence de performance, mais reflète la difficulté du compilateur AOCC à analyser statiquement les structures de pointeurs du projet NPB-CPP. Là où le Fortran atteint 100 \% grâce à sa gestion native des tableaux multidimensionnels, la version C++ introduit un surcoût structurel lié à l’organisation des pointeurs, ce qui limite la portée des optimisations automatiques. Cela confirme que les gains de temps observés résultent principalement de nos directives explicites (\texttt{simd}, \texttt{restrict}) plutôt que d’une optimisation intrinsèque du binaire par le compilateur.
...
\subsection{Expérience d’optimisation sur la version Fortran}
...
\begin{itemize}
    \item Progression : Le temps total d'exécution chute de 31,99 s (1 thread - \texttt{r0}) à 19,56 s (4 threads - \texttt{r2}).
    \item Saturation : À 8 threads (\texttt{r3}), le temps stagne à 19,68 s. Le Scalability Gap de 4,92 indique que l'application sature les ressources mémoire de l'architecture AMD, rendant l'ajout de threads supplémentaire inefficace.
    \item Affinité : Nos réglages \texttt{OMP\_PLACES=cores} ont permis de stabiliser l'affinité à 77,9 \% à 8 threads (\texttt{r3}), contre seulement 23,4 \% initialement sur le run \texttt{r0}.
\end{itemize}

\vspace{0.5cm}

Cette observation conduit à comparer les deux implémentations optimisées afin d’évaluer l’impact réel des transformations appliquées.\\
Le duel entre les deux langages optimisés met en évidence un avantage structurel du Fortran pour ce type de benchmark scientifique.

\inputImage{figures/comparison_duel_report_amd_cpp_vs_f90_opt.png}
{Comparaison des métriques entre Fortran vs C++}
{1.0\textwidth}{!}{fig:comparison_duel_report_amd_cpp_vs_f90_opt}

Cet écart s’explique en partie par une \textit{Array Access Efficiency} plus élevée en Fortran (97,2~\%) ainsi que par un score de compilation optimal (100~\%). Ces indicateurs suggèrent que le compilateur exploite plus efficacement le modèle mémoire et les structures de données propres au Fortran. À l’inverse, la version C++ semble conserver un surcoût structurel, malgré les optimisations manuelles appliquées.\\
